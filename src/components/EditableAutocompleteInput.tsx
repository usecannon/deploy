...
export function EditableAutocompleteInput(props: { items: { label: string, secondary: string }[], defaultValue?: string, tabKeys?: string, placeholder?: string, editable?: boolean, onChange: (item: string) => void, color: string, onPending?: (item: string) => void, onFilterChange?: (text: string) => void }) {
            ...
            <PopoverAnchor>
                <HStack color={props.color} gap={0}>
                    <Editable
                        ...
                        onBlur={finishEdit}
                        onKeyDown={handleKey}
                        onChange={(value) => { setFilterInput(value); if(props.onFilterChange) props.onFilterChange(value) }}
                        value={inputValue}
