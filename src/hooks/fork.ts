...
import { Hex, TransactionRequestBase, createPublicClient, createWalletClient, custom } from "viem"
...
export function useSimulatedTxns(safe: SafeDefinition, txns: (Omit<TransactionRequestBase, 'from'>|null)[]) {
  ...
  const runTxns = async () => {
    ...
    for (const txn of txns) {
      ...
      try {
        const rawEthCall = await node.request({method: 'eth_call', params: [{ from: safe.address, to: txn.to, data: txn.data }]})

        const hash = await node.request({ method: 'eth_sendTransaction', params: [{ from: safe.address, to: txn.to, data: txn.data }]})

        const receipt = await node.request({ method: 'eth_getTransactionReceipt', params: [hash]})
        ...
        results.push({ gasUsed: 0n, callResult: err.data, error: err.toString() })
      }
    }
  ...
  useEffect(() => {
    ...
    if (cleanStateSnapshot && JSON.stringify(txns) !== computedTxns) {
      ...
      node.send('evm_revert', [cleanStateSnapshot]).then(async () => {
        const newCleanState = await node.request({ method: 'evm_snapshot', params: [] })
        try {
          await runTxns()
        } catch(err) {
